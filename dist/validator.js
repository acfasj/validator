(function (global, factory) {
    ...
    class Validator {
        ...
        async validate(data, schema) {
            ...
            const promises = entries.map(([key, rulesOfKey]) => {
                const rules$$1 = formatRules(rulesOfKey);
                return this.validateKey(key, data[key], rules$$1, output); // map 记住要返回一个 promise 啊
            });
         ...
         * @param output
         */
        async validateKey(key, value, rulesOfKey, output) {
            for (const rule of rulesOfKey) {
                ...
                const promises = AsyncFns.map(({ rulename, fn }) => {
                    ...
                    return customPromise.then(null, message => Promise.reject({ message, rulename }));
                });
                // 异步校验
        ...
        checkBuiltin({ rulename, defMessage, value, config, type, }) {
            let checker = rules[rulename];
            let pass = false;
            if (!checker) {
                throw new TypeError('基本规则名不正确, 不存在于内置的RULE_MAP中, 请检查是否拼写错误');
            }
    ...
    Validator.addMessage = addMessage; // 修改 messages 对象
    Validator.addType = addType; // 修改 types 对象
